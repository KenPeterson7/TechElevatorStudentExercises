...
public class JdbcParkDaoTests extends BaseDaoTests {
    ...
    public void getParksByState_returns_empty_list_for_abbreviation_not_in_db() {
        List<Park> parks = sut.getParksByState("EO");
        Assert.assertEquals(0, parks.size());
        //we were expecting an empty list so the expected result is 0 parks in state EO
    }

    @Test
    public void createPark_returns_park_with_id_and_expected_values() {
        Park park = sut.createPark(testPark);
        Integer newId = park.getParkId();
    ...
    public void updated_park_has_expected_values_when_retrieved() {
        ...
        parkToUpdate.setArea(500);
        parkToUpdate.setHasCamping(false);

        sut.updatePark(parkToUpdate);

        Park retrievePark = sut.getPark(1);
        assertParksMatch(parkToUpdate, retrievePark);
    }

    @Test
    public void deleted_park_cant_be_retrieved() {

        sut.deletePark(1);    //delete the park

        Park retrievedPark = sut.getPark(1);    //get the deleted Park by park id
        Assert.assertNull(retrievedPark);    //verify that the park is null or delete
